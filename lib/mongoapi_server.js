...
var CongoServer = function(app){
  ...
  var connect = function(dbName,next){
    var db = mongo.db(hostUrl + "/" + dbName, {safe : true});
    next(db);
  };
  ...
  app.post('/mongo-api/host', function(req, res) {
    hostUrl = req.body.url;
    console.log("hostUrl: " + hostUrl);
    res.json({url: hostUrl});
  });
  
  app.get('/mongo-api/host', function(req, res) {
    res.json({url: hostUrl});
  });
